export const metadata = {
  title: 'getmetatable()',
  description:
    'The getmetatable function in Lua retrieves the metatable of a given object, providing insights into object behavior and structure.',
}
...
- **Accessing the Metatable**: If the object has a metatable without a `__metatable` field, the metatable is returned.
- **Respecting the __metatable Field**: If the object's metatable contains a `__metatable` field, `getmetatable()` returns the value of this field, which restricts direct access to the metatable.
- **No Metatable**: If the object does not have a metatable, `getmetatable()` returns nil.
...
- **Protection via __metatable**: The `__metatable` field acts as a safeguard against unwanted access or modification of a metatable. This is useful in designing robust APIs where you want to prevent external interference with the object's behavior.
- **No Side Effects**: `getmetatable()` does not modify the object or its metatable in any way. It is purely an accessor function.
- **Default Behavior**: Objects like tables and userdata can have metatables. However, Lua's primitive types like number, string, and boolean do not have metatables by default.
- **Combining with setmetatable**: The `getmetatable()` function is often used in conjunction with `setmetatable()` to dynamically inspect and modify object behavior.
